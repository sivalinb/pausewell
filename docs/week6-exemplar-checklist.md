...
| Record exact attack prompts and observed responses | DONE | Pre-NeMo [offline case files](../visitprep_eval/reports/offline-siva/cases/) and [actual Nebius case files](../visitprep_eval/reports/siva-live/cases/). Preserve request-level consent, response and source IDs. |
...
|---|---|---|
| Separate local and live execution | DONE | Pre-NeMo [offline](../visitprep_eval/reports/offline-siva/summary.json): 29 cases, 35 responses, zero remote calls, 28 PASS / 1 WARN. Pre-NeMo [full live](../visitprep_eval/reports/siva-live/summary.json): the same case/response counts and verdicts, with 26 actual provider calls. |
| Keep model outcomes distinct from application outcomes | DONE | Pre-NeMo full live: 24 accepted selections, two empty selections rejected, no timeouts across 26 calls. The historical full run retains 20 accepted / two rejected / four timeouts. A passing final brief does not establish a successful model response. |
...
| Check source fidelity and useful evidence | DONE | Pre-NeMo offline: 127/127 cited facts and 34/34 predefined coverage checks. Pre-NeMo full live: 123/123 and 34/34. Exact copying does not establish source truth, relevance or complete chart coverage. |
...
| Verify hosted observability by readback | DONE | Pre-NeMo safety [Braintrust receipt](../visitprep_eval/reports/siva-live/braintrust.json): 29/29 rows and 26/26 spans. The [paired utility receipt](../visitprep_eval/reports/utility-live-siva/braintrust.json) verifies 16/16 rows and 16/16 spans for each system. Historical receipts remain preserved. |
